		...
		<div class="flex-col text-center lg:flex lg:flex-row lg:justify-between mb-8">
			@empty($category)
				@if(Request::is('category/new'))
					<p class="text-2xl lg:text-3xl border-b-4 border-orange-600">New</p>
				@elseif(Request::is('category/top'))
					<p class="text-2xl lg:text-3xl border-b-4 border-orange-600">Top</p>
				@endif
			@else
				<p class="text-2xl lg:text-3xl border-b-4 border-orange-600">{{ $category->name }}</p>

				<form class="mt-6 lg:mt-0 text-center md:text-right" method="GET" action="/category/{{ $category->name }}/sort">
